...
void insert(char *name, int type, int ln)
{
	int i;
	if(lookup(name))
	{
...
void print()
{
	...
	printf("\n---------------------------------------Symbol Table--------------------------------------\n");
	printf("-----------------------------------------------------------------------------------------\n");
	printf("\nLine\tToken\t\tValue\t\tScope\t\tType\t\tStorage(in bytes)\n");
	printf("-----------------------------------------------------------------------------------------\n");
	for(i=0;i<n;i++)
	{
		if(st[i].type[0]==271)continue;
		if(st[i].type[0]==258)
			printf("%d\t%s\t\t%d\t\t%d\t",st[i].line,st[i].token,(int)st[i].fvalue,st[i].scope);
		else
			printf("%d\t%s\t\t%.1f\t\t%d\t",st[i].line,st[i].token,st[i].fvalue,st[i].scope);
			printf("\t");
		for(j=0;j<st[i].tn;j++)
		{
			if(st[i].type[j]==258){

...
variable_declaration_identifier 
            : identifier {if(duplicate(curid)){printf("Variable redeclared\n");exit(0);}insertSTscope(curid,currscope); ins();  } vdi   
              | array_identifier {if(duplicate(curid)){printf("Variable redeclared\n");exit(0);}insertSTscope(curid,currscope); ins();  } vdi;
...
param_identifier 
            : identifier { ins();insertSTscope(curid,1); params_count++; } param_identifier_breakup;
...
compound_statement 
            : {currscope++;} '{'  statment_list  '}' {deletedata(currscope);currscope--;}  ;
...
void yyerror(char *s)
{
    printf(ANSI_COLOR_RED "%s at line %d: %s\n", s, yylineno, yytext);
    flag=1;
